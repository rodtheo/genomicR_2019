...
# Select all columns except Sepal.Length

select(iris_tidy, -Sepal.Length)
...
```{r}
summarise(iris_tidy, mean_expression = mean(Values, na.rm = TRUE))
```

- We can use function `n()` to count the number of observations

```{r}
summarise(iris_tidy, mean_expression = mean(Values, na.rm = TRUE), count = n())
```
...
iris_tidy %>%
  group_by(Species, Measure_type) %>% 
    summarise(
      least_expr = min(Values),
      most_expr = max(Values)
    )
```
